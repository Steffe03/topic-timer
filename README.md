...
### The timer screen
![TimerPage.png](screenshots/TimerPage.png)

### The list of saved topic sets
![TopicSetsPage.png](screenshots/TopicSetsPage.png)

### The list of topics for a topic set
![TopicsPage.png](screenshots/TopicsPage.png)

### The page for creating new topic sets
![NewTopicsPage.png](screenshots/NewTopicsPage.png)

Pressing '>' gives two new random topics for the board game Tapple.
There's also a light mode version that is enabled if the user's phone is in light mode.

Pressing the timer icon opens the timer screen which does not indicate how much time is left.
The original board game doesn't have a visible timer either. However, since I wanted to make the
game playable without sound (only vibration) the screen flashes orange when it is tapped so the user
